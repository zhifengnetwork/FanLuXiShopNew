...
class Activity extends ApiBase
{
    ...
    public function flash_sale_list()
    {
        ...
        if (!$start_time || !$end_time) {
            $this->ajaxReturn(['status' => -2, 'msg' => '请传入开始时间和结束时间！', 'data' => null]);
        }
        ...
        foreach ($flash_sale_goods as $k => $v) {
            if ($v['item_id']) {
                ...
                if ($info['spec_img']) {
                    $flash_sale_goods[$k]['original_img'] = $info['spec_img'];
                }
    ...
    public function flash_sale_info()
    {
        ...
        if ($info['item_id']) {
            ...
            if ($spe_info['spec_img']) {
                $info['original_img'] = $spe_info['spec_img'];
            }
    ...
    public function post_flash_sale()
    {
        ...
        if (!$id || !$address_id) {
            $this->ajaxReturn(['status' => -2, 'msg' => '参数错误！', 'data' => null]);
        }
        ...
        if ($info['start_time'] > time()) {
            $this->ajaxReturn(['status' => -9, 'msg' => '活动还未开始！', 'data' => null]);
        }

        if ($info['is_end']) {
            $this->ajaxReturn(['status' => -10, 'msg' => '活动已经结束！', 'data' => null]);
        }
        ...
        if (!$address) {
            $this->ajaxReturn(['status' => -4, 'msg' => '请先填写收货地址！', 'data' => null]);
        }

        $Order = M('Order');
        $num = $Order->where(['prom_id' => $id, 'user_id' => $user_id])->count();
        if ($num >= $info['buy_limit']) {
            $this->ajaxReturn(['status' => -6, 'msg' => '您已达到限购件数！', 'data' => null]);
        }
    ...
    public function auction_info()
    {
        ...
        if (!$auction_id) {
            $this->ajaxReturn(['status' => -2, 'msg' => '竞拍参数错误', 'data' => null]);
        }
    ...
    public function group_list()
    {
        ...
        foreach ($list as $k => $v) {
            ...
            if (($v['buy_num'] < 10) && ($v['buy_num'] < $v['virtual_num'])) {
                $list[$k]['num'] = $v['virtual_num'];
            }
